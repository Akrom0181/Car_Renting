...
// @Security ApiKeyAuth
// @Router		/order/{ids} [PUT]
// @Summary		update an order status
// @Description This api updates a order status by its phone and return new status
// @Tags		status
// @Accept		json
// @Produce		json
// @Param		id path string true "order id"
// @Param		order body models.ChangeStatus true "order"
// @Success		200  {string}  string
// @Failure		400  {object}  models.Response
// @Failure		404  {object}  models.Response
// @Failure		500  {object}  models.Response
func (h Handler) UpdateOrderStatus(c *gin.Context) {
	var orderr models.UpdateOrder
	var order models.ChangeStatus
	...
	if _, err := h.Services.Order().ChangeStatus(context.Background(), order); err != nil {
		handleResponseLog(c, h.Log, "error while updating order's status", http.StatusInternalServerError, err.Error())
		return
	}

	handleResponseLog(c, h.Log, "Order's status was successfully updated", http.StatusOK, id)
}
