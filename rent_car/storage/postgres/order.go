...
func (o *OrderRepo) ChangeStatus(ctx context.Context, status models.ChangeStatus) (string, error) {
	order := models.Order{}

	query := `UPDATE status FROM orders WHERE id=$1`
	_, err := o.db.Exec(ctx, query, status.Status)
	if err != nil {
		o.logger.Error("failed to update status in storage", logger.Error(err))
		return "", err
	}
...
func (o *OrderRepo) GetStatus(ctx context.Context, id string) (models.ChangeStatus, error) {

	status := models.CreateOrder{}

	query := `SELECT status FROM orders WHERE id=$1`

	if err := o.db.QueryRow(context.Background(), query, id).Scan(
		&status.Status,
		); err != nil {
		return models.ChangeStatus{}, err
	}
	return models.ChangeStatus{}, nil
}
